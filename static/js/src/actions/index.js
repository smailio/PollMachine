    ...
    return (dispatch, get_state) => {
        ...
        const validate = draft => {
            ...
            if (question.length > 200) {
                errors.question = "Question text too long ! Max is 200 characters.";
            }
            const max_answer_length = Math.max(...answers.map((answer) => answer.text.length));
            if (max_answer_length > 40) {
                errors.answers = "Answer text too long ! Max is 40 characters.";
            }
            return errors;
        };
...
export function save_search_text(text) {
    return {
...
export function receive_search_result(polls) {
...
export function search_in_mine(keywords){
    const index_by_id = polls => _.object(_.map(polls, poll => [poll.poll_id, poll])); // array to object
    return dispatch => {
        return axios
            .get('/api/mine/search', {params: {keywords}})
            .then(response => dispatch(receive_search_result(index_by_id(response.data.polls))));
    }
}